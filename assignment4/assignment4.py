...
task1_with_salary.loc[:, "Salary"] = [7000, 8000, 9000]
...
# Another approach: direct assignemtn with in-place:
...
clean_data.loc[:, "Age"] = pd.to_numeric(clean_data["Age"], errors="coerce") # converting all values in Age to numeric.
...
print("Calculating mean age to use as placeholder for unknown age. Mean age is", mean_age, "\n")
print("Calculating median salary to use as placeholder if salary is unknown. Median salary is", median_salary, "\n")
...
clean_data.loc[:, "Hire Date"] = pd.to_datetime(clean_data["Hire Date"], errors="coerce")
